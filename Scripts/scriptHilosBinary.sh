...
#Script para compilar el algoritmo de busqueda binaria implementado con hilos en un ambiente LINUX
# Compilacion
gcc binary_threads.c tiempo.c -lpthread -lm -o binary
...
for a in ${n[*]}
 do
        ...
        ./binary "$a" "$b" < 10millones.txt  >> "./TiemposBinary.txt"
        done
         #Calculo del promedio de los tiempos obtenidos
      ./promedio 20 < TiemposBinary.txt >> salPromedioBinary.txt
       #Borrado del archivo para el siguiente calculo
       echo " "  > "./TiemposBinary.txt"
 done
